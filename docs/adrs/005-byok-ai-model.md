...
## Context
CloakScan includes AI-enhanced features like code review, explanation, test generation, and refactoring suggestions. These features require Large Language Models (LLMs) which have several cost and privacy considerations:
...
5. **No AI costs for CloakScan** (users control their own spending)
...
# 2. Configure CloakScan
cloakscan config set providers.openai.apiKey sk-proj-...
cloakscan config set providers.openai.model gpt-4

# 3. Use AI features
cloakscan review src/app.ts
cloakscan explain src/utils.ts
cloakscan test-gen src/calculator.ts
```
...
**Traditional (Code through CloakScan backend):**
```
User -> CloakScan Backend -> AI Provider
      ❌ CloakScan sees code
      ❌ Additional hop
      ❌ Potential logging
```
...
### Positive
- **Sustainable**: No AI costs for CloakScan
...
```json
{
  "providers": {
    "openai": {
      "apiKey": "sk-proj-...",  // Stored locally, never sent to CloakScan
      "model": "gpt-4",
...
Or run: cloakscan config set providers.openai.apiKey YOUR_KEY
```
