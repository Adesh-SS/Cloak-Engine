# CloakScan Chat Guide

## Overview

CloakScan Chat is an interactive AI-powered feature that uses RAG (Retrieval-Augmented Generation) to answer questions about your codebase. It provides context-aware responses by retrieving relevant code snippets and documentation from your project.
...
- CloakScan initialized (`cloakscan init`)
- AI provider configured (`cloakscan config`)
...
The first time you run chat, CloakScan will:
...
✓ Conversation exported to: /path/to/parent/directory/cloakscan-chat-chat-abc123-2025-12-06.md
```

**File Location:**

- Saved to the **parent directory** (one level up from your project root)
- Filename format: `cloakscan-chat-{sessionId}-{timestamp}.md`
- Example: `cloakscan-chat-miuk2rhd-leflihb-2025-12-06.md`
...
```bash
cloakscan chat --model gpt-4o
cloakscan chat --model claude-sonnet-4.5
cloakscan chat --model gemini-2.5-flash
```
...
```bash
cloakscan chat --temperature 0.5  # More focused
cloakscan chat --temperature 0.9  # More creative
```
...
```bash
cloakscan chat --embedding-provider openai
```
...
```bash
cloakscan chat --session /path/to/session.json
```
...
```bash
cloakscan chat --export /path/to/my-conversation.md
```
...
1. Check your AI provider connection: `cloakscan status`
2. Verify API key is valid: `cloakscan config`
...
1. Rebuild the index: `cloakscan chat --rebuild`
...
```bash
cloakscan chat --model gpt-4o --temperature 0.5 --rebuild
```
...
Chat works alongside other CloakScan features:

- Use `cloakscan explain` for quick explanations
- Use `cloakscan chat` for interactive exploration
...
- **Your API keys** - Never sent to CloakScan servers
- **Source code** - Never uploaded to CloakScan (only to your AI provider)
