...
All notable changes to CloakScan will be documented in this file.
...
#### AI Model Selection
- **Model Selection in Configuration**: Added ability to select specific AI models during `cloakscan config`:
...
- **Improved Chat Output Formatting**: Enhanced `cloakscan chat` with rich terminal formatting:
...
- **Chat Export Functionality**: Added `/export` command to save conversations:
  ...
  - Format: `cloakscan-chat-{sessionId}-{timestamp}.md`
...
- **Enhanced Config Display**: Improved `cloakscan config` output with:
...
- **Unknown Option Error**: Fixed "unknown option '--debug'" error when using `cloakscan security --debug`. The command now properly accepts and processes the debug flag.
- **Commit Command Flag Mismatch**: Fixed `--no-body` flag not working in `cloakscan commit` command. Commander.js converts `--no-body` to `body: false`, but the handler was checking for `includeBody` property. The command now correctly handles both the `body` property (from `--no-body`) and maintains backward compatibility with `includeBody`.
...
- Updated `DOCKER_GUIDE.md` to document the new `--debug` flag option alongside the existing `CLOAKSCAN_DEBUG` environment variable
...
- **Security Command Debug Flag**: Added `--debug` flag to `cloakscan security` command for verbose debug logging. This provides an alternative to setting the `CLOAKSCAN_DEBUG` environment variable.
  - Usage: `cloakscan security --debug`
  - Automatically sets `CLOAKSCAN_DEBUG=true` when the flag is used
...
CloakScan - 100% Free & Open Source Privacy-First Security Scanning and AI Code Review CLI
...
- Code explainer (`cloakscan explain`)
- AI code review (`cloakscan review`)
- Commit message generator (`cloakscan commit`)
- Documentation generator (`cloakscan docs`)
- Test generator (`cloakscan test-gen`)
- Refactoring suggestions (`cloakscan refactor`)
- Threat modeling (`cloakscan threat-model`)
- Migration assistant (`cloakscan migrate`)
- Interactive RAG-powered chat (`cloakscan chat`)
...
- **Update Source**: Version checks now query `https://registry.npmjs.org/cloakscan/latest` instead of GitHub releases API for more accurate version information.
...
- **Command Options**:
  - Added `--force` option to `cloakscan reset` command
  - Added `--from` and `--to` options to `cloakscan migrate` command
