...
const perfTracker = createPerformanceTracker("cloakscan config");
...
function showConfig(): void {
  ...
  if (config.provider !== "none") {
    ...
    console.log(
      chalk.gray(
        `   No AI provider is configured. CloakScan will use only static analysis tools for code review.\n`
      )
    );
  }
  ...
  console.log(
    chalk.gray(
      `   Unique identifier for this CloakScan installation, used for telemetry and analytics (if enabled).`
    )
  );
  ...
  console.log(
    chalk.gray(
      `   ${
        config.telemetryEnabled
          ? "Usage analytics and error reporting are enabled to help improve CloakScan."
    ...
    chalk.gray(
      `   ${
        config.offlineMode
          ...
          : "CloakScan can connect to external services for updates and telemetry (if enabled)."
      }`
    )
  );
  console.log();
  ...
  console.log(
    chalk.gray(
      `   Timestamp when this CloakScan configuration was first created.`
    )
  );
  ...
  console.log(
    chalk.gray(
      `   Timestamp of the last CloakScan command execution using this configuration.`
    )
  );
    ...
    if (answers.apiKey) {
      ...
      try {
        ...
        if (isAvailable) {
          ...
          console.log(
            chalk.gray(
              "   You can test the connection later with: cloakscan chat"
            )
          );
        }
      } catch (error: any) {
