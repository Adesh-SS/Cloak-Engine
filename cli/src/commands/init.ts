...
const perfTracker = createPerformanceTracker('cloakscan init');
...
export async function initCommand(): Promise<void> {
  ...
  try {
    ...
    if (configExists) {
      ...
      console.log(chalk.gray('\nRun "cloakscan config" to modify settings\n'));
    ...
    if (config.provider === 'none') {
      console.log(chalk.white('  1. Run security scan: ') + chalk.cyan('cloakscan security'));
      console.log(chalk.white('  2. Check status: ') + chalk.cyan('cloakscan status'));
      console.log(chalk.white('  3. Add AI later: ') + chalk.cyan('cloakscan config'));
    } else {
      console.log(chalk.white('  1. Run AI code review: ') + chalk.cyan('cloakscan run'));
      console.log(chalk.white('  2. Run security scan: ') + chalk.cyan('cloakscan security'));
      console.log(chalk.white('  3. Check status: ') + chalk.cyan('cloakscan status'));
    }
...
async function selectSetupMode(): Promise<SetupMode> {
  console.log(chalk.white('How would you like to use CloakScan?\n'));
...
async function setupCloudAI(config: any): Promise<void> {
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      type: 'confirm',
      name: 'telemetry',
      message: 'Enable telemetry? (helps improve CloakScan)',
      default: true,
    },
    {
      type: 'confirm',
...
async function setupStaticOnly(config: any): Promise<void> {
  ...
  console.log(chalk.gray('\nℹ  You can add AI later with: cloakscan config'));
}
