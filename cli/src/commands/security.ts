...
const perfTracker = createPerformanceTracker('cloakscan security');
...
export async function securityCommand(options: SecurityOptions): Promise<void> {
  ...
  if (options.debug || process.env.CLOAKSCAN_DEBUG === 'true') {
...
async function generateAIFixes(
  ...
  try {
    ...
    // Save fixes to file
    const fixesDir = path.join(repoRoot, '.cloakscan', 'fixes');
