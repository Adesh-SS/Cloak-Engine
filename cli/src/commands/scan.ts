...
const perfTracker = createPerformanceTracker('cloakscan scan');
...
export async function scanCommand(options: ScanOptions): Promise<void> {
  ...
  console.log(chalk.cyan.bold('\n🛡️  CloakScan - Comprehensive Security & Quality Analysis\n'));
...
async function runAIReview(results: ScanResults, options: ScanOptions): Promise<void> {
  try {
    ...
    if (config.provider === 'none') {
      ...
      console.log(chalk.gray('   Run `cloakscan config` to set up AI-enhanced reviews\n'));
      return;
    }
    ...
    aiSpinner.info('AI review available via `cloakscan run` command');
...
function generateRecommendations(results: ScanResults): string[] {
  ...
  recommendations.push('Run `cloakscan scan` regularly to catch issues early');
