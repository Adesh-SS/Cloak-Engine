...
const perfTracker = createPerformanceTracker('cloakscan docs');
...
export async function docsCommand(options: DocsCommandOptions): Promise<void> {
  ...
  try {
    ...
    if (docType === 'readme') {
      console.log(chalk.gray('4. Consider generating API docs: cloakscan docs --type api'));
      console.log(chalk.gray('5. Add architecture docs: cloakscan docs --type architecture'));
    }
