...
const perfTracker = createPerformanceTracker('cloakscan migrate');
...
export async function migrateCommand(options: MigrateCommandOptions): Promise<void> {
  ...
  try {
    ...
    if (!config.provider || config.provider === 'none' || !config.apiKey) {
      ...
      console.log(chalk.gray('Configure with: cloakscan config\n'));
      return;
    }
...
function showMigrationOptions(): void {
  ...
  console.log(chalk.gray('\nExamples:'));
  console.log('  cloakscan migrate --type framework --target react-class-to-hooks --report');
  console.log('  cloakscan migrate --type language --target typescript --file src/app.js');
  console.log('  cloakscan migrate --type modernization --target es5-to-es6 --file src/util.js --auto-fix');
  console.log('  cloakscan migrate --type dependency\n');
}
