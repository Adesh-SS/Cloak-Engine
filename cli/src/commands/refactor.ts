...
const perfTracker = createPerformanceTracker('cloakscan refactor');
import {
...
export async function refactorCommand(options: RefactorOptions): Promise<void> {
  ...
  console.log(chalk.blue('\n🔧 CloakScan Refactoring Assistant\n'));
  ...
  if (!config.provider || config.provider === 'none' || !config.apiKey) {
    handleCommandError(new Error('AI provider not configured. Refactoring features require an AI provider. Configure with: cloakscan config'), 'Refactor');
  }
