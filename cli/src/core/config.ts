...
export class ConfigManager {
  ...
  private debug: boolean = process.env.CLOAKSCAN_DEBUG === "true";

  constructor() {
    try {
      // Store config in ~/.cloakscan (with safe home directory resolution)
      const homeDir = getSafeHomeDir();
      this.configDir = path.join(homeDir, ".cloakscan");
  ...
  load(): Config {
    ...
    if (!fs.existsSync(this.configPath)) {
      ...
      throw new Error('Configuration not found. Run "cloakscan init" first.');
    }
