...
export class TelemetryManager {
  ...
  async record(event: Omit<TelemetryEvent, "timestamp">): Promise<void> {
    ...
    if (!config.telemetryEnabled || process.env.CLOAKSCAN_NO_TELEMETRY === 'true') {
      return;
    }
  ...
  async sync(): Promise<void> {
    ...
    if (!config.telemetryEnabled || config.offlineMode || process.env.CLOAKSCAN_NO_TELEMETRY === 'true') {
      return;
    }
...
function initializeTelemetryManager(): TelemetryManager {
  const debug = process.env.CLOAKSCAN_DEBUG === "true";
