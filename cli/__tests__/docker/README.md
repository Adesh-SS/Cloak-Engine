...
This directory contains Docker test infrastructure for validating CloakScan CLI in various containerized environments.
...
```bash
# Build test image
docker build -t cloakscan-test:alpine -f Dockerfile.test ../../

# Run interactive shell
docker run -it --rm cloakscan-test:alpine sh

# Inside container:
npm install -g /path/to/cloakscan.tgz
cloakscan --version
cloakscan security --files 'sample.ts'
```
...
2. **Verify image was built**: `docker images | grep cloakscan-test`
3. **Check package was created**: Ensure `npm pack` was run in cli directory
4. **Enable debug mode**: Set `CLOAKSCAN_DEBUG=true` in test scripts
