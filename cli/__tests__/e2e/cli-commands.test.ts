...
describe("CLI Commands E2E", () => {
  ...
  beforeAll(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "cloakscan-e2e-"));
...
function complexFunction(x) {
  ...
  describe("Config Management", () => {
    it("should initialize config", () => {
      const configPath = path.join(tempDir, ".cloakscan", "config.json");
  ...
  describe("Status Command", () => {
    it("should show status information", () => {
      try {
        ...
        expect(output).toBeDefined();
        expect(output.toLowerCase()).toContain("cloakscan");
      } catch (error: any) {
