...
describe("Config Lifecycle Integration", () => {
  ...
  beforeEach(() => {
    testConfigDir = path.join(os.tmpdir(), `cloakscan-test-${Date.now()}`);
    originalHome = process.env.CLOAKSCAN_HOME;
    originalDebug = process.env.CLOAKSCAN_DEBUG;
    // Set env var before any imports that might use it
    process.env.CLOAKSCAN_HOME = testConfigDir;
  ...
  afterEach(() => {
    if (originalHome !== undefined) {
      process.env.CLOAKSCAN_HOME = originalHome;
    } else {
      delete process.env.CLOAKSCAN_HOME;
    }
    if (originalDebug !== undefined) {
      process.env.CLOAKSCAN_DEBUG = originalDebug;
    } else {
      delete process.env.CLOAKSCAN_DEBUG;
    }
    if (fs.existsSync(testConfigDir)) {
      fs.rmSync(testConfigDir, { recursive: true, force: true });
    }
  ...
  it("should complete full config lifecycle: init -> load -> update -> reset", async () => {
    ...
    // the default path. The test will still work if CLOAKSCAN_HOME is set correctly.
    // Verify the config directory matches our test directory
    const expectedConfigDir = path.join(testConfigDir, ".cloakscan");
