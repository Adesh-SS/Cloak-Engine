# CloakScan Performance Guide

This guide explains CloakScan's performance characteristics, how to profile commands, and optimization tips.
...
```bash
export CLOAKSCAN_PROFILE=true
cloakscan scan 2>&1 | tee perf.log
```
...
CloakScan caches AI responses and analysis results:
...
cloakscan security --files src/**/*.ts

# Scan specific directory
cloakscan scan --files src/
```
...
```bash
# Skip tests
cloakscan scan --skipTests

# Skip performance testing
cloakscan scan --skipPerf
```
...
- Use `.cloakscanignore` to exclude large files
...
```bash
# Before changes
cloakscan scan > before.log

# After changes
cloakscan scan > after.log
...
cloakscan run 2>&1 | grep -i "network\|api\|http"
```
...
CloakScan can use **k6** for load and stress testing of APIs and web services.

### When to Use k6

Use `cloakscan perf` with k6 when:
...
# Run load test
cloakscan perf --load --url https://api.example.com

# Run stress test
cloakscan perf --stress --url https://api.example.com
```
