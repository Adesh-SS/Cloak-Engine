...
| `cloakscan perf`     | Performance testing & load testing |
| `cloakscan mutation` | Mutation testing for test quality  |
...
CloakScan requires the following runtime dependencies (automatically installed with npm):

- **TypeScript** (`typescript`) - Required for AST parsing of TypeScript/JavaScript files
  - Automatically included when installing via `npm install -g cloakscan`
