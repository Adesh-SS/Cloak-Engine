...
We actively support the following versions of CloakScan with security updates:
...
We take security vulnerabilities seriously. If you discover a security vulnerability in CloakScan, please follow these steps:
...
1. **Keep CloakScan Updated**

   ```bash
   npm update -g cloakscan
   ```

2. **Verify Installation**
   - Only install from official npm registry: `npm install -g cloakscan`
   - Verify package integrity: `npm audit cloakscan`
...
5. **Network Security**
   - CloakScan works offline for static analysis
...
- CloakScan does not store or log your API keys
...
CloakScan scans your dependencies for known vulnerabilities:
...
CloakScan includes several built-in security features:
...
We appreciate the security research community's efforts to keep CloakScan secure. Security researchers who responsibly disclose vulnerabilities will be credited (unless they prefer anonymity).
