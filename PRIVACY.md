...
CloakScan is committed to protecting your privacy. This Privacy Policy explains how we collect, use, and protect information when you use CloakScan.
...
2. **No code is uploaded** to CloakScan servers
...
# Or manually edit: ~/.cloakscan/config.yaml
...
```bash
export CLOAKSCAN_NO_TELEMETRY=true
cloakscan security
```
...
- **Stored locally** in your configuration file (`~/.cloakscan/config.yaml`)
- **Never transmitted** to CloakScan servers
...
- **CloakScan does NOT see** your code or AI responses
...
CloakScan uses the following third-party services:
...
- **Cache**: Stored locally, can be cleared with `cloakscan reset`
...
CloakScan is not intended for users under 13 years of age. We do not knowingly collect personal information from children.
...
4. **Use CloakScan completely offline** (no telemetry, no network)

---

## 📋 Summary

**What CloakScan Collects (if telemetry enabled):**
...
**What CloakScan Does NOT Collect:**
...
**CloakScan is committed to privacy-first development. Your code stays yours.**
